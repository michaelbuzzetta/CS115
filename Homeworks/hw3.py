...
def indexTrack(coins):
    '''Returns the index of the largest coin'''
    if len(coins) <= 1:
        return 0
    elif coins[0] > coins[1]:
        return indexTrack([coins[0]] + coins[2:])
    else:
        return 1 + indexTrack(coins[1:])

'''
this function returns the smallest possible combo of coins
'''
def changeHelper(amount, realAmount, coins):
    ind = indexTrack(coins)
    
    if amount == 0 or coins == []:
        return []
    elif coins[ind] > amount:
        return changeHelper(amount, realAmount, coins[0:ind] + coins[ind+1:])
    else:
        use = [coins[ind]] + changeHelper(amount - coins[ind], realAmount, coins)
        lose = changeHelper(amount, realAmount, coins[0:ind] + coins[ind+1:])
...
def giveChange(amount, coins):
    best = changeHelper(amount, amount, coins)

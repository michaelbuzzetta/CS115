...
def FA(x,y,cin):
    ...
    i=XOR(m,0)
    s=XOR(XOR(x,y),cin)
    cout=gor3(gand(x,y),gand(y,cin),gand(x,cin))
    return (cout,s)
...
def fourBitAdd(xxxx,yyyy):
    ...
    (a,ze)=FA(xxxx[1],yyyy[1],d)
    (b,zf)=FA(xxxx[0],yyyy[0],a)
    return (b,(zf,ze,zt,zo))
...
def test_fourBitAdd():
    ...
    fourteen=(1,1,1,0)
    fifteen=(1,1,1,1)
    a,bb=fourBitAdd(zero,one)
    assert(bb ==(0,0,0,1) and a==0)
    a,bb=fourBitAdd(one,two)
    assert(bb==(0,0,1,1) and a==0)
    a,bb=fourBitAdd(two,three)
    assert(bb==(0,1,0,1) and a==0)
    a,bb=fourBitAdd(three,four)
    assert(bb==(0,1,1,1) and a==0)
